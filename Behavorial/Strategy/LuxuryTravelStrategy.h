...
class LuxuryTravelStrategy : public TravelStrategy
{
...
HotelBooking LuxuryTravelStrategy::SelectHotel(const std::string& Location, std::vector<HotelBooking> Hotels)
{
    ...
    auto MostExpensive = std::max_element(Hotels.begin(), GoodLocations, [](auto H1, auto H2){ return H1.Price < H2.Price; });
    assert(MostExpensive != Hotels.end());
    return *MostExpensive;
}
    
TransportTicket LuxuryTravelStrategy::SelectTransport(const std::string& From,
                                                      const std::string& To,
                                                      std::vector<TransportTicket> Transports)
{
    ...
    auto MostExpensive = std::max_element(Transports.begin(), GoodItineraries, [](auto T1, auto T2){ return T1.Price < T2.Price; });
    assert(MostExpensive != Transports.end());
    return *MostExpensive;
}